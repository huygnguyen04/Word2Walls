...
class TextureMapper:
    ...
    def export_textured_mesh(self, filename="output/3d_textured_model.obj"):
        ...
        try:
            # First, export just the mesh without material reference
            self.mesh.export(filename)
            
            # Then manually add the MTL reference to the OBJ file
            with open(filename, 'r') as f:
                obj_content = f.read()
                
            with open(filename, 'w') as f:
                # Add MTL reference at the beginning of the file
                f.write(f"mtllib {mtl_basename}\n")
                # Write the original content
                f.write(obj_content)
                
            print(f"Successfully exported textured mesh to {filename} with materials")
            return filename
        except Exception as e:
            print(f"Error exporting textured mesh: {e}")
            # Fallback to basic export without materials
            self.mesh.export(filename)
            print(f"Exported mesh without materials to {filename}")
            return filename 